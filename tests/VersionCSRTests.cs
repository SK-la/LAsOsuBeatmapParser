...
namespace LAsOsuBeatmapParser.Tests
{
    ...
    public class VersionCSRTests
    {
        ...
        public void TestVersionCSRSingleFile_MultipleFiles()
        {
            ...
            var results = new List<(string fileName, string version, double sr, long timeMs, long memoryBytes)>();
            ...
            foreach (string filePath in MultipleTestFiles)
            {
                ...
                foreach ((string versionName, Func<Beatmap, double> calculator) in calculators)
                {
                    ...
                    results.Add((fileName, versionName, sr, stopwatch.ElapsedMilliseconds, memoryUsed));
            ...
            // 计算统计信息
            IEnumerable<IGrouping<string, (string fileName, string version, double sr, long timeMs, long memoryBytes)>> versionGroups = results.GroupBy(r => r.version);

            foreach (IGrouping<string, (string fileName, string version, double sr, long timeMs, long memoryBytes)> group in versionGroups)
            {
        ...
        // 辅助方法：使用V3.0-Before版本计算
        private double CalculateWithV30Before(Beatmap beatmap)
        {
            var calculator = new SRCalculatorV30Before();
