...
namespace LAsOsuBeatmapParser.Tests
{
    ...
    public class ComparisonAllVersionTests
    {
        ...
        public void TestComparisonSingleFile_Once()
        {
            ...
            _output.WriteLine($"OD值: {beatmap.BeatmapInfo.Difficulty.OverallDifficulty}");
            ...
            foreach ((string algorithm, double sr, long timeMs, long memoryBytes) in results)
            {
                ...
                // _output.WriteLine($"{algorithm}: SR={sr:F4}, Time={timeMs}ms, Memory={memoryMB:F2}MB");
            }

            _output.WriteLine(table.ToString());
            _output.WriteLine("");

            // Assert - 验证算法结果一致性（允许小误差）
            // 只比较FromFile版本：C# FromFile, Rust FromFile, Python FromFile
            var fromFileAlgorithms = new[] { "C# FromFile", "Rust FromFile", "Python FromFile" };
            var fromFileResults = results.Where(r => fromFileAlgorithms.Contains(r.algorithm)).ToList();

            if (fromFileResults.Count >= 2)
            {
                double referenceSR = fromFileResults.First().sr;
        ...
        public void TestComparisonMultipleFiles_Once()
        {
            ...
            {
                Assert.True(sr <= 15, $"SR值过高 (CS{cs} - {algorithm}): {sr}");
            }
