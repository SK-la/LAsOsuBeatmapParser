...
impl SRCalculator {
    ...
    const CROSS_MATRIX: &'static [&'static [f64]] = &[
        &[-1.0],
        &[0.075, 0.075],
        &[0.125, 0.05, 0.125],
    ...
    pub fn calculate_sr(beatmap: &Beatmap) -> Result<f64, String> {
        ...
        let mut note_sequence: Vec<Note> = beatmap.hit_objects.iter().map(|ho| {
            let col = ho.position.x as i32; // Since we set position.x = col
            let mut time = ho.start_time;
            let mut tail = if ho.end_time > ho.start_time { ho.end_time } else { -1 };
            if time < 0 { time = 0; }
            if tail < -1 { tail = -1; }
        ...
        let t = note_sequence.iter().map(|n| n.h).max().unwrap().max(
            note_sequence.iter().map(|n| n.t).max().unwrap()
        ) + 1;
        ...
        let (a_bar, ks): (Vec<f64>, Vec<i32>) = Self::calculate_section_26(&delta_ks, k, t, &note_sequence);
        let (r_bar, _): (Vec<f64>, Vec<f64>) = Self::calculate_section_27(&ln_seq_sorted, &ln_seq_sorted, t, &note_seq_by_column, x);

        let result = Self::calculate_section_3(j_bar, x_bar, p_bar, a_bar, r_bar, ks, t, &note_sequence, &ln_seq_sorted);
    ...
    pub fn calculate_sr_from_data(data: &SRData) -> Result<f64, String> {
        ...
        let mut note_sequence: Vec<Note> = data.hit_objects.iter().map(|ho| {
        ...
        let mut ln_seq: Vec<Note> = note_sequence.iter().filter(|n| n.t >= 0).cloned().collect();
        ...
        let mut max_t = 0;
        for note in &note_sequence {
            if note.h > max_h { max_h = note.h; }
            if note.t > max_t { max_t = note.t; }
        }
        ...
        let (a_bar, ks): (Vec<f64>, Vec<i32>) = Self::calculate_section_26(&delta_ks, k, t, &note_sequence);
        let (r_bar, _): (Vec<f64>, Vec<f64>) = Self::calculate_section_27(&ln_seq, &ln_seq, t, &note_seq_by_column, x);

        let result = Self::calculate_section_3(j_bar, x_bar, p_bar, a_bar, r_bar, ks, t, &note_sequence, &ln_seq);

        Ok(result)
    }

    fn calculate_section_23(k: i32, note_seq_by_column: &[Vec<Note>], t: i32, x: f64) -> (Vec<f64>, Vec<Vec<f64>>) {
        ...
        j_ks.par_iter_mut().zip(delta_ks.par_iter_mut()).enumerate().for_each(|(k_idx, (j_k, delta_k))| {
            if k_idx < note_seq_by_column.len() && note_seq_by_column[k_idx].len() > 1 {
                for i in 0..note_seq_by_column[k_idx].len() - 1 {
                    let delta = 0.001 * (note_seq_by_column[k_idx][i + 1].h - note_seq_by_column[k_idx][i].h) as f64;
        ...
        let j_bar_ks: Vec<Vec<f64>> = j_ks.par_iter().map(|jk| Self::smooth2(jk, t)).collect();
    ...
    fn calculate_section_24(k: i32, t: i32, note_seq_by_column: &[Vec<Note>], x: f64) -> Vec<f64> {
        ...
        for k_idx in 0..=(k as usize) {
            let notes_in_pair = if k_idx == 0 {
                ...
                let right_col = k_idx;
                let mut left_notes = note_seq_by_column.get(left_col).cloned().unwrap_or_default();
                let mut right_notes = note_seq_by_column.get(right_col).cloned().unwrap_or_default();
        ...
        for s in (0..t as usize).step_by(Self::GRANULARITY as usize) {
            x_arr[s] = 0.0;
            for k_idx in 0..=(k as usize) {
                x_arr[s] += x_ks[k_idx][s] * Self::CROSS_MATRIX[k as usize][k_idx];
            }
        }
        ...
        for i in 0..note_seq.len() - 1 {
            ...
            if delta < 1e-9 {
                if (note_seq[i].h as usize) < p.len() {
                    p[note_seq[i].h as usize] += 1000.0 * (0.02 * (4.0 / x - Self::LAMBDA_3)).sqrt().sqrt();
                }
            } else {
                let h_l = note_seq[i].h as usize;
                let h_r = note_seq[i + 1].h as usize;
                let v = 1.0 + lambda2_scaled * (prefix_sum_ln_bodies[h_r] - prefix_sum_ln_bodies[h_l]);

                let base_val = if delta < 2.0 * x / 3.0 {
                    (0.08 / x * (1.0 - Self::LAMBDA_3 / x * (delta - x / 2.0) * (delta - x / 2.0))).sqrt().sqrt() * b(delta) * v / delta
                } else {
                    (0.08 / x * (1.0 - Self::LAMBDA_3 / x * (x / 6.0) * (x / 6.0))).sqrt().sqrt() * b(delta) * v / delta
                };
    ...
    fn calculate_section_26(delta_ks: &[Vec<f64>], k: i32, t: i32, note_seq: &[Note]) -> (Vec<f64>, Vec<i32>) {
        ...
        note_seq.par_iter().for_each(|note| {
            ...
            // Note: This is not thread-safe, but for simplicity we'll use sequential processing
        });

        // Sequential processing for ku_ks to avoid borrow checker issues
        for note in note_seq {
            ...
            for s in start_time..=end_time {
                if note.k >= 0 && (note.k as usize) < ku_ks.len() && s < ku_ks[note.k as usize].len() {
        ...
        for s in (0..t as usize).step_by(Self::GRANULARITY as usize) {
            ...
            for i in 0..cols.len().saturating_sub(1) {
                let col1 = cols[i];
                let col2 = cols[i + 1];
                if col1 >= delta_ks.len() || col2 >= delta_ks.len() || s >= delta_ks[col1].len() || s >= delta_ks[col2].len() || col1 >= dks.len() || s >= dks[col1].len() {
                    continue;
                }
        ...
        let a_bar = Self::smooth2(&a, t);
        (a_bar, ks)
    }

    fn calculate_section_27(ln_seq: &[Note], tail_seq: &[Note], t: i32, note_seq_by_column: &[Vec<Note>], x: f64) -> (Vec<f64>, Vec<f64>) {
        ...
        i_arr.par_iter_mut().enumerate().for_each(|(i, i_val)| {
            ...
            let column_notes = note_seq_by_column.get(k as usize).cloned().unwrap_or_default();
        ...
        for i in 0..tail_seq.len().saturating_sub(1) {
            ...
            if delta_r <= 0.0 {
                continue; // Skip invalid delta_r
            }
            let is_val = 1.0 + i_arr[i];
            let r_val = 0.08 * delta_r.powf(-0.5) * x.powf(-1.0) * (1.0 + Self::LAMBDA_4 * (i_arr[i] + i_arr.get(i + 1).copied().unwrap_or(0.0)));
    ...
    fn calculate_section_3(j_bar: Vec<f64>, x_bar: Vec<f64>, p_bar: Vec<f64>, a_bar: Vec<f64>, r_bar: Vec<f64>, ks: Vec<i32>, t: i32, note_seq: &[Note], ln_seq: &[Note]) -> f64 {
        ...
        for time in 0..t as usize {
            ...
            let term1 = Self::W_0 * (a_bar_val.powf(3.0 / ks[time] as f64) * j_bar_val).powf(1.5);
            let term2 = (1.0 - Self::W_0) * (a_bar_val.powf(2.0 / 3.0) * (0.8 * p_bar_val + r_bar_val)).powf(1.5);
            s[time] = (term1 + term2).powf(2.0 / 3.0);

            let t_t = a_bar_val.powf(3.0 / ks[time] as f64) * x_bar_val / (x_bar_val + s[time] + 1.0);
        ...
        let weight_sum = c.iter().sum::<f64>();
        let weighted_sum: f64 = d.iter().map(|&val| val.powf(Self::LAMBDA_N)).zip(c.iter()).map(|(d_pow, &c_val)| d_pow * c_val).sum();

        let mut sr = (weighted_sum / weight_sum).powf(1.0 / Self::LAMBDA_N);

        sr = sr.powf(Self::P_0) / 8.0f64.powf(Self::P_0) * 8.0;
        let total_notes = note_seq.len() as f64 + ln_seq.iter().map(|n| ((n.t - n.h).min(1000) as f64) / 400.0).sum::<f64>();
    ...
    fn smooth2(lst: &[f64], t: i32) -> Vec<f64> {
        let mut lst_bar = vec![0.0; t as usize];
        let mut window_sum = 0.0;
        let mut window_len = 0;

        // Initialize window for s = 0
        let initial_window = std::cmp::min(500, t);
        for i in (0..initial_window).step_by(Self::GRANULARITY as usize) {
            window_sum += lst[i as usize];
            window_len += Self::GRANULARITY;
        }

        for s in (0..t as usize).step_by(Self::GRANULARITY as usize) {
            lst_bar[s] = window_sum / window_len as f64 * Self::GRANULARITY as f64;

            // Add new element to window
            let next_s = s as i32 + 500;
            if next_s < t {
                let next_idx = next_s as usize;
                if next_idx < lst.len() {
                    window_sum += lst[next_idx];
                    window_len += Self::GRANULARITY;
                }
            }

            // Remove old element from window
            let prev_s = s as i32 - 500;
            if prev_s >= 0 {
                let prev_idx = prev_s as usize;
                if prev_idx < lst.len() {
                    window_sum -= lst[prev_idx];
                    window_len -= Self::GRANULARITY;
                }
            }
        }
    ...
    fn smooth(lst: &[f64], t: i32) -> Vec<f64> {
        ...
        for s in (0..t as usize).step_by(Self::GRANULARITY as usize) {
            ...
            let right = ((s as i32 + 500).min(t - 1) + 1) as usize;
...
pub extern "C" fn calculate_sr_from_json(json_ptr: *const u8, len: usize) -> *mut u8 {
    ...
    let json_bytes = unsafe {
        std::slice::from_raw_parts(json_ptr, len)
    };
...
pub extern "C" fn calculate_sr_from_struct(data: *const CBeatmapData) -> f64 {
    ...
    let hit_objects: Vec<HitObject> = unsafe {
        std::slice::from_raw_parts(c_data.hit_objects_ptr, c_data.hit_objects_count)
    }.iter().map(|ho| HitObject {
        ...
        for line in content.lines() {
            ...
            if in_difficulty_section {
                if let Some((key, value)) = parse_key_value(line) {
                    match key.as_str() {
                        "OverallDifficulty" => {
                            overall_difficulty = value.parse().map_err(|_| "Invalid OverallDifficulty")?;
                        }
                        "CircleSize" => {
